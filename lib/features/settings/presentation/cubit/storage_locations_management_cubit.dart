...
class StorageLocationsManagementCubit
    extends Cubit<StorageLocationsManagementState> {
  ...
  Future<List<String>> getSupportedItemTypeIds(String storageLocationId) async {
    try {
      return await _storageLocationRepository.getSupportedItemTypeIds(
        storageLocationId,
      );
    } catch (_) {
      return [];
  ...
  Future<bool> createStorageLocation({
    ...
    if (trimmedName.isEmpty) {
      emit(
        state.copyWith(errorMessage: AppStrings.storageLocationNameRequired),
      );
      return false;
    }
    ...
    try {
      ...
      final locations = await _storageLocationRepository.getAllLocations();
      emit(state.copyWith(isActionInProgress: false, locations: locations));
      return true;
    } on Failure catch (f) {
