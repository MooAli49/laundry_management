...
class StorageLocationsDao extends DatabaseAccessor<app_db.AppDatabase> {
  ...
  Future<void> insertLocation(
    app_db.StorageLocationsCompanion companion,
  ) async {
  ...
  Future<void> replaceSupportedItemTypes(
    ...
    )..where((t) => t.storageLocationId.equals(storageLocationId))).go();
  ...
  Future<List<String>> getSupportedItemTypeIds(String storageLocationId) async {
    final query = selectOnly(db.storageLocationItemTypes)
      ..where(
        db.storageLocationItemTypes.storageLocationId.equals(storageLocationId),
      )
      ..addColumns([db.storageLocationItemTypes.itemTypeId]);
    final rows = await query.get();
    return rows
        .map((r) => r.read(db.storageLocationItemTypes.itemTypeId)!)
        .toList();
  }

  Future<List<app_db.StorageLocation>> getCompatibleLocationsForItemType(
    String itemTypeId,
  ) async {
    final query =
        select(db.storageLocations).join([
            innerJoin(
              db.storageLocationItemTypes,
              db.storageLocationItemTypes.storageLocationId.equalsExp(
