-- =============================================================================
-- Migration: 20260923000001_restrict_edit_order_to_processing.sql
-- Description: Edit Processing Order V1 — Phase 2 Correction:
--              Restricts sync_update_order_aggregate to strictly 'processing' orders.
--              Orders in 'ready', 'completed', or 'cancelled' status are rejected
--              with INVALID_LIFECYCLE_TRANSITION (ERRCODE 23514).
--              Preserves all other validations, idempotency, carpet handling,
--              storage deletion guard (P0006), and single sync_changes full-aggregate append.
-- =============================================================================
...
BEGIN
    ...
    -- 3. Strict Processing-Only Lifecycle Guard
    IF v_existing.status <> 'processing' THEN
        RAISE EXCEPTION 'INVALID_LIFECYCLE_TRANSITION: Only orders in processing status can be edited (current status: %)',
