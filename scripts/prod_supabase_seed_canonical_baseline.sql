-- =============================================================================
-- GUARDED PRODUCTION SCRIPT: CANONICAL MASTER BASELINE SEEDING
...
SET LOCAL app.project_ref = 'rvrskluqfbrkvvlxtxfp';
...
BEGIN
    IF v_project_ref IS NULL OR v_project_ref <> 'rvrskluqfbrkvvlxtxfp' THEN
        RAISE EXCEPTION 'PRECONDITION FAILED: app.project_ref must be set to rvrskluqfbrkvvlxtxfp. Aborting.';
    END IF;

    IF v_environment IS NULL OR v_environment <> 'production' THEN
        RAISE EXCEPTION 'PRECONDITION FAILED: app.environment must be set to production. Aborting.';
    END IF;

    IF v_confirmation IS NULL OR v_confirmation <> 'CONFIRMED_CANONICAL_SEED_2026_PROD_RVRSKLUQFBRKVVLXTXFP' THEN
