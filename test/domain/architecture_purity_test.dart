...
void main() {
  group('Domain Layer Purity Architecture Test', () {
    test(
      'lib/domain/ has zero imports from Flutter, Drift, SQLite, Dio, or Core Failures',
      () {
            ...
            if (line.trim().startsWith('import ') ||
              ...
              for (final pattern in prohibitedPatterns) {
                if (line.contains(pattern)) {
                  violations.add(
                    '${file.path}:${i + 1} contains prohibited import: $line',
                  );
                }
              }
            }
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Domain layer must remain 100% pure Dart without infrastructure dependencies:\n${violations.join('\n')}',
        );
      },
    );
  });
