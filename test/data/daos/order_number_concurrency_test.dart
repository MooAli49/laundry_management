...
void main() {
  ...
  domain.Order createDraftOrder({required String id, String orderNumber = ''}) {
  ...
  group('Order Number Concurrency & Format & Immutability', () {
    test(
      'order numbers follow YY-XXX format strictly and sequential generation',
      () async {
        final now = DateTime.now();
    ...
    test('order number collision triggers entire transaction retry and succeeds', () async {
      ...
      final testOrdersDao = _CollidingOrdersDao(
        db,
        collidingNumber: collidingNumber,
      );
      ...
      expect(testOrdersDao.generatedNumbers, [
        collidingNumber,
        '$yearPrefix-002',
      ]);
      ...
      final items = await (db.select(
        db.orderItems,
      )..where((t) => t.orderId.equals('ord-new'))).get();
      expect(items.length, 1);
      expect(items.first.id, 'item-new');

      // Verify sync operation was recorded only once
      final syncOps = await (db.select(
        db.syncOperations,
      )..where((t) => t.entityId.equals('ord-new'))).get();
      expect(syncOps.length, 1);
    });

    test(
      'order number collision exhausts 5 retries and rolls back transaction',
      () async {
        final now = DateTime.now();
        final yearPrefix = (now.year % 100).toString().padLeft(2, '0');
        final collidingNumber = '$yearPrefix-001';
        ...
        final alwaysCollidingDao = _AlwaysCollidingOrdersDao(
          db,
          collidingNumber: collidingNumber,
        );
        final testOrderRepo = OrderRepositoryImpl(
        ...
        final items = await (db.select(
          db.orderItems,
        )..where((t) => t.orderId.equals('ord-fail'))).get();
        expect(items, isEmpty);
      },
    );

    test(
      'rapid / concurrent order creations succeed without collisions or duplicates',
      () async {
        final now = DateTime.now();
        final yearPrefix = (now.year % 100).toString().padLeft(2, '0');
