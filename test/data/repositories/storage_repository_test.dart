...
void main() {
  ...
  group('StorageRepositoryImpl — Queries, Search, Filters & Pagination', () {
    test(
      '17. getItemsRequiringStorageWithDetails returns unstored items for active orders',
      () async {
        final items = await repository.getItemsRequiringStorageWithDetails(
          limit: 50,
          offset: 0,
        );
    ...
    test(
      '18. getCurrentStorageItems returns items with active storage records',
      () async {
        final items = await repository.getCurrentStorageItems(
          limit: 50,
          offset: 0,
        );
        expect(items, isNotEmpty);
    ...
    test(
      '19. Search filters items by order number, customer name, and customer phone',
      () async {
        // Find an unstored item to test search
        final allUnstored = await repository
        ...
        expect(
          byOrderNum.any((i) => i.orderItem.id == target.orderItem.id),
          true,
        );
        ...
        expect(
          byCustName.any((i) => i.orderItem.id == target.orderItem.id),
          true,
        );

        // Search by customer phone
        final byCustPhone = await repository
            .getItemsRequiringStorageWithDetails(query: target.customerPhone);
        expect(
          byCustPhone.any((i) => i.orderItem.id == target.orderItem.id),
          true,
        );
    ...
    test(
      '20. Location filter returns only items in the specified location',
      () async {
    ...
    test('21. Pagination limit and offset work correctly', () async {
      final page1 = await repository.getItemsRequiringStorageWithDetails(
        limit: 2,
        offset: 0,
      );
      final page2 = await repository.getItemsRequiringStorageWithDetails(
        limit: 2,
        offset: 2,
      );
      ...
      if (page1.isNotEmpty && page2.isNotEmpty) {
        expect(
          page1.first.orderItem.id,
          isNot(equals(page2.first.orderItem.id)),
        );
      }
    });
    ...
    test('22. Count queries accurately reflect total matching items', () async {
      ...
      final listRequiring = await repository
          .getItemsRequiringStorageWithDetails(limit: 500);
  ...
  group('StorageRepositoryImpl — Store, Move & Unstore Atomic Operations', () {
    test(
      'Store valid item creates active StorageRecord and logs SyncOperation',
      () async {
        final unstored = await repository.getItemsRequiringStorageWithDetails();
        final target = unstored.first;

        // Find compatible location
        final compatible = await storageLocationsDao
            .getCompatibleLocationsForItemType(target.orderItem.itemTypeId);
        ...
        expect(
          syncOps.any(
            (op) => op.entityId == record.id && op.operationType == 'create',
          ),
          true,
        );

        // Verify item now appears in current storage and not in requiring storage
        final active = await repository.getActiveRecordForOrderItem(
          target.orderItem.id,
        );
        expect(active, isNotNull);
        expect(active!.storageLocationId, loc.id);

        final requiringAfter = await repository
            .getItemsRequiringStorageWithDetails();
        expect(
          requiringAfter.any((i) => i.orderItem.id == target.orderItem.id),
          false,
        );
      },
    );
    ...
    test(
      '25. Atomic Move: moves item and deactivates previous active record',
      () async {
        final current = await repository.getCurrentStorageItems();
        final target = current.first;
        ...
        final allRecordsForOldLoc = await storageRecordsDao
            .getActiveRecordsForLocation(oldLocationId);
        expect(
          allRecordsForOldLoc.any((r) => r.orderItemId == target.orderItem.id),
          false,
        );
    ...
    test(
      'Move rejects moving to same storage location with BusinessRuleFailure',
      () async {
        final current = await repository.getCurrentStorageItems();
        final target = current.first;
    ...
    test('26. Atomic Bulk Storage stores multiple items atomically', () async {
      ...
      final item2 = unstored.firstWhere(
        (i) =>
            i.orderItem.itemTypeId == item1.orderItem.itemTypeId &&
            i.orderItem.id != item1.orderItem.id,
        orElse: () => unstored[1],
      );

      final compatible = await storageLocationsDao
          .getCompatibleLocationsForItemType(item1.orderItem.itemTypeId);
      final loc = compatible.first;
      ...
      final active1 = await repository.getActiveRecordForOrderItem(
        item1.orderItem.id,
      );
      final active2 = await repository.getActiveRecordForOrderItem(
        item2.orderItem.id,
      );
    ...
    test(
      '27. Unstore persistence: deactivates active record, item returns to requiring storage',
      () async {
        final current = await repository.getCurrentStorageItems();
        final target = current.first;
        ...
        final activeAfter = await repository.getActiveRecordForOrderItem(
          target.orderItem.id,
        );
        expect(activeAfter, isNull);

        // Verify item returned to requiring storage
        final requiring = await repository
            .getItemsRequiringStorageWithDetails();
        expect(
          requiring.any((i) => i.orderItem.id == target.orderItem.id),
          true,
        );
        ...
        expect(
          currentAfter.any((i) => i.orderItem.id == target.orderItem.id),
          false,
        );
        ...
        expect(
          syncOps.any(
            (op) =>
                op.entityId == target.activeRecord!.id &&
                op.operationType == 'unstore',
          ),
          true,
        );
      },
    ...
    test(
      'Unstore rejects already unstored item with BusinessRuleFailure',
      () async {
        final unstored = await repository.getItemsRequiringStorageWithDetails();
        final target = unstored.first;
    ...
    test(
      'Unstore from Ready order automatically reverts Order to Processing, and storing it again makes Order Ready',
      () async {
        ...
        final orders = await ordersDao.getOrders(
        ...
        final allInitiallyStored = await repository.areAllOrderItemsStored(
          readyOrder.id,
        );
        expect(allInitiallyStored, isTrue);

        final firstItemId = orderItems.first.id;
        final activeRecord = await repository.getActiveRecordForOrderItem(
          firstItemId,
        );
        ...
        final recordAfter = await repository.getActiveRecordForOrderItem(
          firstItemId,
        );
        ...
        final compatibleLocs = await storageLocationRepo
            .getCompatibleLocationsForItemType(orderItems.first.itemTypeId);
        final storeResult = await storeUseCase.execute(
    ...
    test('Unstore from Processing order preserves Processing status', () async {
      ...
      final processingItem = currentStored.firstWhere(
        (i) => i.orderStatus == OrderStatus.processing,
      );
    ...
    test(
      'Unstore from Completed or Cancelled order throws BusinessRuleFailure',
      () async {
        final currentStored = await repository.getCurrentStorageItems();
