...
void main() {
  ...
  group('PaymentRepositoryImpl Hardening Tests', () {
    test(
      'payment on Completed order is rejected with BusinessRuleFailure',
      () async {
        expect(
    ...
    test(
      'payment on Cancelled order is rejected with BusinessRuleFailure',
      () async {
        expect(
          () => paymentRepository.recordPayment(
            createPayment(
    ...
    test(
      'exact remaining amount is allowed and leaves remaining balance at 0',
      () async {
    ...
    test(
      'multiple payments allowed up to remaining and overpayment after previous payments is rejected',
      () async {
        ...
        final finalTotalPaid = await paymentsDao.getTotalPaidForOrder(
          'ord-active',
        );
    ...
    test(
      'getPaymentSummariesForOrders returns batch paid and remaining amounts accurately in a single grouped query',
      () async {
        await paymentRepository.recordPayment(
          createPayment(
            id: 'pay-batch-1',
        ...
        final summaries = await paymentRepository.getPaymentSummariesForOrders([
          'ord-active',
          'non-existent',
        ]);
