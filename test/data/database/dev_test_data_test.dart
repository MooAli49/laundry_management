...
void main() {
  group('DevTestData & Production Seed Safety Tests', () {
    test(
      ...
      () async {
        ...
        expect(
          customers.isEmpty,
          isTrue,
          reason: 'Production seed must NOT create customers',
        );
        ...
        expect(
          orders.isEmpty,
          isTrue,
          reason: 'Production seed must NOT create orders',
        );
        ...
        expect(
          orderItems.isEmpty,
          isTrue,
          reason: 'Production seed must NOT create order items',
        );
        ...
        expect(
          payments.isEmpty,
          isTrue,
          reason: 'Production seed must NOT create payments',
        );
        ...
        expect(
          ...
          isTrue,
          reason: 'Production seed must NOT create storage records',
        );
        ...
        expect(
          expenses.isEmpty,
          isTrue,
          reason: 'Production seed must NOT create expenses',
        );

        final syncOperations = await db.select(db.syncOperations).get();
        expect(
          syncOperations.isEmpty,
          isTrue,
          reason: 'Production seed must NOT create sync operations',
        );
    ...
    test(
      'DevTestData populates required scenarios with exactly 12 customers and 18 orders',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        ...
        expect(
          statuses,
          containsAll(['processing', 'ready', 'completed', 'cancelled']),
        );
        ...
        final ord2Items = await (db.select(
          db.orderItems,
        )..where((t) => t.orderId.equals(ord2.id))).get();
        ...
        final ord4Payments = await (db.select(
          db.payments,
        )..where((t) => t.orderId.equals(ord4.id))).get();
        ...
        final ord5Payments = await (db.select(
          db.payments,
        )..where((t) => t.orderId.equals(ord5.id))).get();
    ...
    test(
      'repeated execution of DevTestData is idempotent and does NOT duplicate records',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
    ...
    test(
      'E-Wallet payment from DevTestData successfully travels through domain mapping to PaymentMethod.ewallet',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        await DevTestData.seedDevData(db);
        ...
        final ord16Payments = await paymentRepository.getPaymentsForOrder(
          ord16.id,
        );
        expect(ord16Payments, isNotEmpty);
        expect(
          ord16Payments.first.paymentMethod,
        ...
        final ord11 = orders.firstWhere((o) => o.orderNumber == '26-011');
        final ord11Payments = await paymentRepository.getPaymentsForOrder(
          ord11.id,
        );
        final ewalletPayments11 = ord11Payments
            .where((p) => p.paymentMethod == PaymentMethod.ewallet)
            .toList();
        expect(ewalletPayments11, isNotEmpty);
        expect(
          ewalletPayments11.first.paymentMethod,
          equals(PaymentMethod.ewallet),
        );
        ...
        final ord18Payments = await paymentRepository.getPaymentsForOrder(
          ord18.id,
        );
        final ewalletPayments18 = ord18Payments
            .where((p) => p.paymentMethod == PaymentMethod.ewallet)
            .toList();
        expect(ewalletPayments18, isNotEmpty);
        expect(
          ewalletPayments18.first.paymentMethod,
          equals(PaymentMethod.ewallet),
        );
    ...
    test(
      'DevTestData storage location compatibility is internally consistent and seeded',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        await DevTestData.seedDevData(db);
        ...
        final supportedTypes = await storageLocationsDao
            .getSupportedItemTypeIds(DevTestData.locRackA1Id);
        expect(
          supportedTypes.contains(DevTestData.typeClothingId),
          isTrue,
          reason: 'locRackA1Id must be compatible with typeClothingId',
        );

        // Verify clothes compatible locations (Rack A1, Rack A2, Rack B1)
        final clothingLocations = await storageLocationsDao
            .getCompatibleLocationsForItemType(DevTestData.typeClothingId);
        final clothingLocIds = clothingLocations.map((l) => l.id).toList();
        expect(
          clothingLocIds,
          containsAll([
            DevTestData.locRackA1Id,
            DevTestData.locRackA2Id,
            DevTestData.locRackB1Id,
          ]),
        );
        ...
        final carpetLocations = await storageLocationsDao
            .getCompatibleLocationsForItemType(DevTestData.typeCarpetsId);
        ...
        final blanketLocations = await storageLocationsDao
            .getCompatibleLocationsForItemType(DevTestData.typeBlanketsId);
        ...
        final orders = await db.select(db.orders).get();
        final ord1 = orders.firstWhere((o) => o.orderNumber == '26-001');
        final ord1Items = await (db.select(
          db.orderItems,
        )..where((t) => t.orderId.equals(ord1.id))).get();
        final unstoredClothing = ord1Items.firstWhere(
          (i) => i.itemTypeId == DevTestData.typeClothingId,
        );
        ...
        final record = await storageRepo.getActiveRecordForOrderItem(
          unstoredClothing.id,
        );
    ...
    test(
      'StorageRepository rejects incompatible storage location with Arabic error message',
      () async {
        final db = AppDatabase(NativeDatabase.memory());
        await DevTestData.seedDevData(db);
        ...
        final clothingItem = ord1Items.firstWhere(
          (i) => i.itemTypeId == DevTestData.typeClothingId,
        );
