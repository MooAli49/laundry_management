...
void main() {
  group('PhoneUtils', () {
    test('normalizePhoneNumber trims leading and trailing whitespace', () {
      expect(
        PhoneUtils.normalizePhoneNumber('  01012345678  '),
        equals('01012345678'),
      );
      expect(
        PhoneUtils.normalizePhoneNumber('\t01123456789\n'),
        equals('01123456789'),
      );
    });

    test(
      'normalizePhoneNumber converts Eastern Arabic-Indic numerals (٠-٩) to Western (0-9)',
      () {
        expect(
    ...
    group('isValidCustomerPhone (Egyptian Mobile Rule)', () {
      ...
      test(
        'rejects numbers with wrong length, invalid prefixes, or international codes',
        () {
          // Wrong length
