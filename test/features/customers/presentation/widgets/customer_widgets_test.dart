...
void main() {
  group('Customer Widgets Tests', () {
    testWidgets(
      'CustomerCard renders customer info, order count and handles tap',
      (tester) async {
        bool tapped = false;
        ...
        final item = CustomerListItemViewModel(
          customer: customer,
          orderCount: 3,
        );

        await tester.pumpWidget(
          testBoilerplate(CustomerCard(item: item, onTap: () => tapped = true)),
        );
    ...
    testWidgets(
      'CustomerFormDialog renders add form and validates required fields',
      (tester) async {
        String? savedName;
        String? savedPhone;
    ...
    testWidgets('CustomerFormDialog pre-populates fields when editing', (
      tester,
    ) async {
      final now = DateTime.now();
      final customer = Customer(
