...
void main() {
  ...
  group('CustomerDetailCubit', () {
    test(
      'loadCustomerDetail populates customer data, orders, and balances',
      () async {
    ...
    test(
      'activeOrdersCount strictly counts processing and ready; completed and cancelled are not active',
      () async {
        final now = DateTime.now();
        final customer = await customerRepository.createCustomer(
          Customer(
    ...
    test(
      'loadCustomerDetail accurately calculates totalPaid, totalRemaining, and latestOrder',
      () async {
        final now = DateTime.now();
        final customer = await customerRepository.createCustomer(
          Customer(
    ...
    test('updateCustomerInfo updates customer details successfully', () async {
      ...
      expect(
        cubit.state.data!.customer.name,
        equals('سعيد عبد الله بعد التعديل'),
      );
    ...
    test(
      'updateCustomerInfo with duplicate phone emits error and does not update',
      () async {
        final now = DateTime.now();
    ...
    test(
      'customer with >20 orders has authoritative full aggregate counts, paginated history, and working loadMore',
      () async {
        final now = DateTime.now();
        final customer = await customerRepository.createCustomer(
          Customer(
        ...
        for (var i = 0; i < 25; i++) {
          ...
          if (targetStatus != OrderStatus.processing) {
            await ordersDao.updateOrderStatus(
              ...
              cancellationReason: targetStatus == OrderStatus.cancelled
                  ? 'سبب الإلغاء'
                  : null,
              updatedAt: now,
            );
    ...
    test('loadCustomerDetail clears actionSuccessMessage on reload', () async {
      ...
      expect(
        cubit.state.actionSuccessMessage,
        equals('تم تحديث بيانات العميل بنجاح'),
      );
    ...
    test(
      'unexpected exceptions emit localized generic Arabic error message',
      () async {
