...
void main() {
  ...
  group('CustomersScreen Tests', () {
    testWidgets(
      'renders header, search bar, and empty state when no customers exist',
      (tester) async {
    ...
    testWidgets('renders list of customer cards when customers exist', (
      tester,
    ) async {
    ...
    testWidgets(
      'renders 2-column grid on tablet and 1-column on narrow width',
      (tester) async {
        final repo = getIt<CustomerRepository>();
        final now = DateTime.now();
        await repo.createCustomer(
    ...
    testWidgets(
      'creates a customer and refreshes list via Cubit without direct Repository access',
      (tester) async {
        await tester.pumpWidget(testBoilerplate(const CustomersScreen()));
        await tester.pumpAndSettle();
        ...
        await tester.enterText(
          textFields.at(1),
          'جمال عبد الناصر',
        ); // First field in dialog (0 is search bar)
    ...
    testWidgets(
      'displays load-more button when customer count > 50 and appends next page on tap',
      (tester) async {
        final repo = getIt<CustomerRepository>();
        final now = DateTime.now();
        for (var i = 1; i <= 55; i++) {
