...
import 'package:laundry_management/data/local/database/app_database.dart' hide Order, OrderItem, Customer, StorageLocation, StorageRecord, Payment;
...
void main() {
  ...
  group('COMPLETE E2E ORDER LIFECYCLE & GATES AUDIT', () {
    test('Normal E2E 26-step lifecycle: processing -> store items -> auto-ready -> pay remaining -> complete with handover', () async {
      final now = DateTime.now();
      ...
      expect(createdOrder.customerNameSnapshot, equals('عميل التدقيق الكامل'));
      ...
      expect(createdOrder.customerPickupFee, equals(const Money.fromPiastres(1500)));
      expect(createdOrder.customerDeliveryFee, equals(const Money.fromPiastres(2000)));
      ...
      final remainingAfterDown = await paymentRepository.getRemainingAmountForOrder(createdOrder.id);
      expect(remainingAfterDown, equals(const Money.fromPiastres(7500))); // 75 EGP
      ...
      final record1 = await storageRecordsDao.getActiveRecordForOrderItem(item1.id);
      ...
      final orderAfterItem1 = await orderRepository.getOrderById(createdOrder.id);
      ...
      final record2 = await storageRecordsDao.getActiveRecordForOrderItem(item2.id);
      ...
      final orderAfterItem2 = await orderRepository.getOrderById(createdOrder.id);
      ...
      final remainingAfterFinal = await paymentRepository.getRemainingAmountForOrder(createdOrder.id);
      expect(remainingAfterFinal, equals(Money.zero));
      final totalPaid = await paymentRepository.getTotalPaidForOrder(createdOrder.id);
      expect(totalPaid, equals(const Money.fromPiastres(12500)));

      // 19. Verify payment appears in payment history
      final paymentHistory = await paymentRepository.getPaymentsForOrder(createdOrder.id);
      ...
      final activeRecord1After = await storageRecordsDao.getActiveRecordForOrderItem(item1.id);
      final activeRecord2After = await storageRecordsDao.getActiveRecordForOrderItem(item2.id);
      ...
      final allStorage1 = await (db.select(db.storageRecords)
            ..where((t) => t.orderItemId.equals(item1.id)))
          .get();
      final allStorage2 = await (db.select(db.storageRecords)
            ..where((t) => t.orderItemId.equals(item2.id)))
          .get();
      ...
      Future<String> createGateOrder({
        ...
        if (storeItems) {
          ...
          await db.customStatement(
            'INSERT INTO storage_records (id, order_item_id, storage_location_id, is_active, created_at, updated_at) '
            'VALUES (?, ?, ?, 1, ?, ?);',
            ['rec-$orderId', 'gate-item-$orderId', 'loc-audit-1', nowTimestamp, nowTimestamp],
          );
        }
    ...
    test('Cancellation Flow: Mandatory reason, historical payments preserved, storage released, no further actions', () async {
      final now = DateTime.now();
      ...
      await db.customStatement(
        'INSERT INTO storage_records (id, order_item_id, storage_location_id, is_active, created_at, updated_at) '
        'VALUES (?, ?, ?, 1, ?, ?);',
        ['rec-canc-$orderId', 'canc-item-$orderId', 'loc-audit-1', nowTimestamp, nowTimestamp],
      );
      ...
      expect(cancelledOrder.cancellationReason, equals('العميل ألغى الطلب لظروف طارئة'));
      ...
      final activeRecord = await storageRecordsDao.getActiveRecordForOrderItem('canc-item-$orderId');
      expect(activeRecord, isNull);
      final allStorage = await (db.select(db.storageRecords)
            ..where((t) => t.orderItemId.equals('canc-item-$orderId')))
          .get();
      expect(allStorage.length, equals(1));
      expect(allStorage.first.isActive, isFalse);
