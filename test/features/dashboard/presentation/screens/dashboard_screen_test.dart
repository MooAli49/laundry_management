...
void main() {
  ...
  Widget buildTestableWidget(Widget child) {
    return MaterialApp(locale: const Locale('ar'), home: child);
  }

  group('DashboardScreen Widget & Specification Tests', () {
    testWidgets(
      'renders header, quick actions, operational summary, attention, and sections',
      (tester) async {
        ...
        expect(
          find.byKey(const ValueKey('dashboard_add_order_button')),
          findsOneWidget,
        );
        expect(
          find.text('إضافة طلب'),
          findsWidgets,
        ); // quick action and possible empty state button

        expect(
          find.byKey(const ValueKey('dashboard_add_customer_button')),
          findsOneWidget,
        );
        expect(find.text('إضافة عميل'), findsOneWidget);

        expect(
          find.byKey(const ValueKey('dashboard_record_payment_button')),
          findsOneWidget,
        );
        expect(find.text('تسجيل دفعة'), findsOneWidget);

        expect(
          find.byKey(const ValueKey('dashboard_add_expense_button')),
          findsOneWidget,
        );
        ...
        expect(
          find.byKey(const ValueKey('attention_storage_item')),
          findsOneWidget,
        );
        expect(
          find.byKey(const ValueKey('attention_unpaid_item')),
          findsOneWidget,
        );
        expect(
